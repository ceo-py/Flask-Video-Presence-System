...
// Grid Classes
const CLASS_GRID_RESPONSIVE = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6";
...
const heartbeatBlob = new Blob([heartbeatWorkerScript], { type: 'application/javascript' });
...
async function init() {
    try {
        ...
        if (cameras.length > 0) {
            ...
            grid.innerHTML = '<div class="col-span-full text-center text-slate-500">No cameras detected.</div>';
        }
    } catch (e) {
        console.error("Initialization failed", e);
        grid.innerHTML = '<div class="col-span-full text-center text-red-500">System Offline. Connection Failed.</div>';
    }
}
...
function loadStream(video, camName, loader) {
    ...
    startStream(camName).then(async data => {
    ...
    if (document.visibilityState === 'visible') {
        ...
        Object.values(hlsInstances).forEach(hls => {
            if (hls.media) {
                ...
                if (hls.latency > 3) {
                     hls.media.currentTime = hls.liveSyncPosition || (hls.media.duration - 1);
                }
